...
//! `filesystems`/`hostdevs`/`serials`/`tpms` are modeled so far, matching
//! how many device types have their own struct ([`super::DeviceDisk`],
...
//! [`super::DeviceTpm`]) — the remaining `NewDevice` variants (ticket 12)
//! get their own `Vec<T>` field here as each one is implemented, same
//! pattern.
//!
...
use super::{
    ...
    DeviceNetwork, DeviceSerial, DeviceSound, DeviceTpm,
};
