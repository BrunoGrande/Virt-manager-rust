...
- [Async / background-task model](./issues/09-async-background-task-model.md) — ticket 09, open. Spun out of ticket 08: tokio-bridged-into-GLib vs `glib::MainContext::spawn_local` vs OS threads + `glib::idle_add`, plus the shared cancellable-progress-modal wrapper every long-running call routes through (`asyncjob.py` equivalent).
- Per-screen specs for the remaining 16 screens + 13 shared widgets ticket 07 located — next up: VM details window, Add Hardware wizard, VM Manager main window.
